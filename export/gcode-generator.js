...
(function() {
    ...
    class GCodeGenerator {
        ...
        // REVIEW - Are all these defensive checks necessary? It's not like the processors are registered anywhere else?
        ...
        generate(toolpathPlans, options) {
            ...
            if (options.includeComments && processorName !== 'roland') {
                ...
                commentBlock.push(
                    (c.header || 'G-code generated by {app}').replace('{app}', appName)
                );
        ...
        linearizeArc(cmd, startPos, resolution = 1.0) {
            ...
            if (cmd.type === 'ARC_CW') {
                if (sweep >= EPSILON) sweep -= 2 * Math.PI;
            } else {
                if (sweep <= -EPSILON) sweep += 2 * Math.PI;
            }

            // Handle full circle
            // REVIEW - Does it make sense to add a double check sweep angle, something like:
            // if (Math.abs(sweep) < PRECISION) {
            //     const chordLenSq = (endPoint.x - startPoint.x) ** 2 + (endPoint.y - startPoint.y) ** 2;
            //     if (chordLenSq < PRECISION * PRECISION) {
            //         sweep = arc.clockwise ? -2 * Math.PI : 2 * Math.PI;
            //     } else {
            //         // Micro-arc: treat as linear, don't promote to full circle
            //         sweep = 0;
            //     }
            // }

            const dist = Math.hypot(start.x - end.x, start.y - end.y);
            if (dist < PRECISION && Math.abs(sweep) < EPSILON) {
